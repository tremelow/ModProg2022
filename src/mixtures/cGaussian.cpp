...
namespace Mixtures {
	...
	/*!
	 * \fn cGaussian& cGaussian::operator=(const cGaussian& thecGaussian)
	 * \param const cGaussian::cGaussian& thecGaussian: source
	 * \detail Copy argument into *this
	 */
        cGaussian& cGaussian::operator=(const cGaussian& thecGaussian)
        {
            copy(thecGaussian);
            return (*this);
        }
        
	/* cDistribution* cGaussian::PtrCopy() const
	{
		cGaussian *mycGaussian = NULL ;

		mycGaussian = new cGaussian();
                mycGaussian->copy(*this);
	...
	/*!
	 * \fn ostream& operator<<(ostream &os, const cGaussian& thecGaussian)
	 * \param ostream& theOut: the output stream, default cout.
	 * \param const cGaussian& thecGaussian: object to print
	 */
        ostream& operator<<(ostream &os, const cGaussian& thecGaussian)
        { 
            os << "Gaussian distribution with mean "
               << thecGaussian.parameter[0] << " and variance " 
               << thecGaussian.parameter[1] << endl ;
