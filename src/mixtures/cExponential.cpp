...
namespace Mixtures {
	...
	/*!
	 * \fn cExponential& cExponential::operator=(const cGaussian& thecExponential)
	 * \param const cExponential::cGaussian& thecGaussian: source
	 * \detail Copy argument into *this
	 */
        cExponential& cExponential::operator=(const cExponential& thecExponential)
        {
            copy(thecExponential);
            return (*this);
        }
	...
	void cExponential::Generate(uint theNSample, vector<double>& theYt) const 
	{
        ...
        /* if you want to use the global RNG */
        // for (uint t = 0 ; t < theNSample ; t++)
        //     theYt[t] = gsl_ran_exponential(rng.get(), parameter[0]);
	...
	/*!
	 * \fn ostream& operator<<(ostream &os, const cExponential& thecExponential)
	 * \param ostream& theOut: the output stream, default cout.
	 * \param const cExponential& thecExponential: object to print
	 */
        ostream& operator<<(ostream &os, const cExponential& thecExponential)
        { 
            os << "Exponential distribution with parameter "
                        << thecExponential.parameter[0] << endl ;
            return os ;
        }
